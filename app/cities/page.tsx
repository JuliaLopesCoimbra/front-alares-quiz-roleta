    ...
    ? Object.fromEntries(
        Object.entries(data)
          .map(([uf, cidades]) => [
            uf,
            busca
              ? cidades.filter(c => c.toLowerCase().includes(busca.toLowerCase()))
              : cidades,
          ])
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb', padding: '40px 32px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <input
          ...
          style={{
            border: '1px solid #e5e7eb', borderRadius: 10, padding: '12px 18px',
            fontSize: '1rem', outline: 'none', width: '100%', maxWidth: 400,
          ...
          {Object.entries(dadosFiltrados).map(([uf, cidades]) => (
            ...
            >
              ...
              <div style={{
                ...
                <div>
                  <span style={{ fontSize: '1.3rem', fontWeight: 800, color: '#065f46' }}>{uf}</span>
                  <span style={{ fontSize: '0.9rem', color: '#6b7280', marginLeft: 10 }}>
                    {ESTADO_NOMES[uf] ?? ''}
                  </span>
                </div>
                <span style={{
